...
function dhtmlXCarousel(conf, effect, skin) {
	...
	if (typeof(window.addEventListener) == "function") {
		...
		if (window.navigator.pointerEnabled == true) { // IE11
			this.conf.dnd_evs = {
				start: "pointerdown",
				move: "pointermove",
				end: "pointerup"
		...
		this._doOnTouchStart = function(e) {
			...
			if (window.dhx4.isIE10 && e.pointerType == e.MSPOINTER_TYPE_MOUSE || window.dhx4.isIE11 && e.pointerType == "mouse") {
				return;
			}
			...
			if (e.type.match(/^touch/) != null) {
				...
				that.area.style.msTouchAction = "none";
				that.conf.touch_conf.x = e.clientX;
				that.conf.touch_conf.y = e.clientY;
			}
			
			window.addEventListener(that.conf.dnd_evs.move, that._doOnTouchMove, false);
			window.addEventListener(that.conf.dnd_evs.end, that._doOnTouchEnd, false);
		...
		this._doOnTouchEnd = function(e) {
			...
			window.removeEventListener(that.conf.dnd_evs.move, that._doOnTouchMove, false);
			window.removeEventListener(that.conf.dnd_evs.end, that._doOnTouchEnd, false);
		...
		this.area.addEventListener(this.conf.dnd_evs.start, this._doOnTouchStart, false);
	...
	this.unload = function() {
		...
		if (typeof(window.addEventListener) == "function") {
			...
			this.area.removeEventListener(this.conf.dnd_evs.start, this._doOnTouchStart, false);
		} else {
...
dhtmlXCarousel.prototype._initControls = function() {
	...
	this._doOnControlClick = function(e) {
		...
		e = e||event;
		if (that.conf.dnd_evs != null && e.type != "click" && that.conf.clear_click != true) { // force animation on touch devices by touch
			if (window.dhx4.isIE10 && e.pointerType == e.MSPOINTER_TYPE_MOUSE || window.dhx4.isIE11 && e.pointerType == "mouse") {
				return; // prevent pointer on desktop in IE
			}
	...
	if (typeof(window.addEventListener) == "function") {
		...
		if (this.conf.dnd_evs != null) {
			this.controls.addEventListener(this.conf.dnd_evs.start, this._doOnControlClick, false);
		}
	} else {
	...
	this._unloadControls = function() {
		...
		if (typeof(window.addEventListener) == "function") {
			...
			if (this.conf.dnd_evs != null) this.controls.removeEventListener(this.conf.dnd_evs.start, this._doOnControlClick, false);
		} else {
